...
<?php if($_SESSION['userid'] < 1000): ?><div class="layout ws10 mac content afcf settingBox">
  <div class="mt30 bc2box br10 p30 ofh">
    ...
    <div class="ws10 fr bdl9 pl30 sright">
      <div class="tar"><a class="bc3box pt5 pb5 pl20 pr20 fs14 tac dib br3" href="<?php echo U('Company/add_company');?>">添加部门</a></div>
      <table width="100%" cellspacing="0" cellpadding="0" class="table tableBlue mt10">
          <tr>
            <th class="nwp">部门名称</th>
            <th class="nwp">操作</th>
          </tr>
		  <?php if(is_array($companyList)): $i = 0; $__LIST__ = $companyList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo['zhcompanytype'] != '无'): ?><tr>
            <td><?php echo ($vo["zhcompanytype"]); ?></td>
            <td><a class="ml5" href="<?php echo U('Company/update_company',array('companyid'=>$vo['id']));?>">修改信息</a> | <a class="ml5" url="<?php echo U('Company/del_company',array('companyid'=>$vo['id']));?>" onclick="delRecord($(this).attr('url'));">删除部门</a></td>
...
<script type="text/javascript" src="/Public/js/companytypelist.js"></script>
